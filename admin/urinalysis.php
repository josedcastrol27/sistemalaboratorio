...
<?php
	...
	$qjan = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Jan' && `year` = '$year'") or die(mysqli_error());
	$fjan = $qjan->fetch_array();
	$qfeb = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Feb' && `year` = '$year'") or die(mysqli_error());
	$ffeb = $qfeb->fetch_array();
	$qmar = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Mar' && `year` = '$year'") or die(mysqli_error());
	$fmar = $qmar->fetch_array();
	$qapr = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Apr' && `year` = '$year'") or die(mysqli_error());
	$fapr = $qapr->fetch_array();
	$qmay = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'May' && `year` = '$year'") or die(mysqli_error());
	$fmay = $qmay->fetch_array();
	$qjun = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Jun' && `year` = '$year'") or die(mysqli_error());
	$fjun = $qjun->fetch_array();
	$qjul = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Jul' && `year` = '$year'") or die(mysqli_error());
	$fjul = $qjul->fetch_array();
	$qaug = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Aug' && `year` = '$year'") or die(mysqli_error());
	$faug = $qaug->fetch_array();
	$qsep = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Sept' && `year` = '$year'") or die(mysqli_error());
	$fsep = $qsep->fetch_array();
	$qoct = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Oct' && `year` = '$year'") or die(mysqli_error());
	$foct = $qoct->fetch_array();
	$qnov = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Nov' && `year` = '$year'") or die(mysqli_error());
	$fnov = $qnov->fetch_array();
	$qdec = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `month` = 'Dec' && `year` = '$year'") or die(mysqli_error());
	...
	<div id = "content">
		...
		<div id = "target">
			...
			<table id = "table" cellspacing = "0" class = "display">
				...
				<?php 
					$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
					$query = $conn->query("SELECT * FROM `urinalysis` NATURAL JOIN `itr` GROUP BY `itr_no` ORDER BY `urinalysis_id` DESC") or die(mysqli_error());
				...
				?>
					<tr>
						...
						<td><center><a class = "btn btn-primary" href = "urinalysis_record.php?itr_no=<?php echo $fetch['itr_no']?>"><span class = "glyphicon glyphicon-search"></span> Todos los Registros</a></center></td>
					</tr>
				<?php
					}
				?>		
