...
<body>
	...
	<div id = "content">
		...
		<?php
			...
			$f1 = $q1->fetch_array();
		?>
		<div class = "alert alert-info">Xray Record / <?php echo $f1['firstname']." ".substr($f1['middlename'], 0, 1).". ".$f1['lastname']?><a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "xray.php"><span class = "glyphicon glyphicon-hand-right"></span> VOLVER</a></div>
		<table id = "table" cellspacing = "0" class = "display">
			...
			<?php 
				$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
				$q = $conn->query("SELECT * FROM `radiological` NATURAL JOIN `itr` WHERE `itr_no` = '$_GET[itr_no]' GROUP BY `itr_no` ORDER BY `rad_id` DESC") or die(mysqli_error());
			...
			?>
				<tr>
					...
					<td><center><a class = "btn btn-info" href = "xray_form.php?itr_no=<?php echo $f['itr_no']?>&rad_id=<?php echo $f['rad_id']?>"><span class = "glyphicon glyphicon-search"></span> VER DETALLES</a><center></td>
				</tr>
			<?php
				}
			?>	
