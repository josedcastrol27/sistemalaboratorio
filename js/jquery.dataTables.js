...
(/** @lends <global> */function( $ ) {
	 ...
	 *    with client-side processing (Opcional)
	 *  @memberof DataTable#oApi
	 */
	 ...
	 *
	 * @return {integer} Current page length. Note `-1` indicates that Todos los Registros
	 *   are to be shown.
	 *//**
	 * Set the current page length.
	 *
	 * @param {integer} Page length to set. Use `-1` to show Todos los Registros.
	 * @returns {DataTables.Api} this
	 */
	...
	var __row_selector = function ( settings, selector, opts )
	{
		return _selector_run( selector, function ( sel ) {
			...
			// Todos los Registros, so no need to check if the index is in there, since it
	...
	DataTable.defaults = {
		...
		"oLanguage": {
			 ...
			 *      $('#example').dataTable( {
			 *        "language": {
			 *          "infoPostFix": "Todos los Registros shown are derived from real information."
			 *        }
			 *      } );
			 *    } );
			 */
	...
	DataTable.ext = _ext = {
		 ...
		 * parameter that is used for the column (si la tuviera).
		 *
		 * The functions defined take two parameters:
		 *
		 * 1. `{object}` DataTables settings object: see
